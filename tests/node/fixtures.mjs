/** Deliberate loop fixtures for node:test (sandbox runner). */

/** Exact trailing unit ×3 (unit len ≥5). */
export const EN_CONSECUTIVE_HIT = "Preface. " + "hello".repeat(3);

/** Chinese exact trailing unit ×3 (5+ chars each). */
export const ZH_CONSECUTIVE_HIT = "前言。" + "你好世界啊".repeat(3);

/** Line spam: identical non-empty lines ×3 (len≥4; total ≥15 for length gate). */
export const EN_LINE_HIT = ["Emit GO!", "Emit GO!", "Emit GO!"].join("\n");

/** Chinese line spam — pad so length gate clears. */
export const ZH_LINE_HIT = ["继续输出吧", "继续输出吧", "继续输出吧"].join("\n");

/**
 * Variation stem spam without exact trailing consecutive units.
 * Interleave unique tails so consecutive/line do not fire first.
 */
...
/** One-char-per-line collapse (≥40). Unique ending breaks consecutive. */
export const EN_CHAR_SPLIT_HIT =
  Array.from({ length: 48 }, (_, i) => "abcdefghijklmnopqrstuvwxyz"[i % 26]).join(
    "\n",
  ) + "\nEND";

export const ZH_CHAR_SPLIT_HIT =
  Array.from({ length: 48 }, (_, i) => "甲乙丙丁戊己庚辛壬癸"[i % 10]).join("\n") +
  "\n结束标记";

/**
 * Broken shell draft scoring ≥8.
 */
...
export const ZH_BROKEN_SHELL_HIT = "草稿如下：\n" + EN_BROKEN_SHELL_HIT;

/**
 * Density phrase repeated with unique glue so consecutive does not win.
 */
...
/** Low 4-gram diversity; tests pass maxUnitLen=4 so consecutive cannot run. */
